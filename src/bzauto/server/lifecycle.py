...
async def start_server(host: str | None = None, port: int | None = None) -> None:
    ...
    uvi_config = uvicorn.Config(app, host=h, port=p, log_level="warning")
    _server = uvicorn.Server(uvi_config)
    _server_task = asyncio.create_task(_server.serve())
    log.info("服务器已启动: http://%s:%s/socket.io/", h, p)
...
async def ensure_tab(
    ...
    *,
    ...
    while not registry.is_connected(account_id) if account_id else not registry.is_connected():
    ...
    if url:
        if reuse_existing:
            tabs = registry.get_tabs(account_id) if account_id else registry.tabs
            for tab in tabs:
                if tab.get("url") == url:
