...
class BossScrapeFlow:
        ...
        *,
        ...
        for scroll in range(max_scrolls):
            ...
            for i, card in enumerate(cards):
                ...
                bbox = await self._page._session.bbox(
                    select="a.op-btn-chat",
                    filter={"nth": len(all_jobs) - 1},
                )
                if bbox:
                    await self._page._session.click(bbox["physical"]["cx"], bbox["physical"]["cy"])
    ...
    async def _send_greeting(self) -> bool:
        try:
            chat_btn = await self._page._session.bbox("a.op-btn-chat")
            if chat_btn:
                await self._page._session.click(chat_btn["physical"]["cx"], chat_btn["physical"]["cy"])
    ...
    async def _dismiss_dialogs(self) -> None:
        for selector in [
            ...
            try:
                bbox = await self._page._session.bbox(selector)
                if bbox:
